 ...
 * of type ``void*``, to be passed to all user-facing OSStep routines
 * listed below.  If unsuccessful, a ``NULL`` pointer will be
 * returned, and an error message will be printed to ``stderr``.
 */
void* OSStepCreate(OSRhsFn, f, realtype t0, N_Vector y0, SunContext sunctx);

void OSStepFree(void** arkode_mem);

// Tolerances
int OSStepSStolerances(void* arkode_mem, realtype reltol, realtype abstol);

int OSStepSVtolerances(void* arkode_mem, realtype reltol, N_Vector abstol);

int OSStepWFtolerances(void* arkode_mem, OSEwtFn efun);

int OSStepResStolerance(void* arkode_mem, realtype rabstol);

int OSStepResVtolerance(void* arkode_mem, N_Vector rabstol);

int OSStepResFtolerance(void* arkode_mem, OSRwtFn rfun);

// Main function
int OSStepEvolve(void* arkode_mem, realtype tout, N_Vector yout, realtype *tret, int itask);

// Set Interpolator
int OSStepSetInterpolantType(void* arkode_mem, int itype);

int OSStepSetInterpolantDegree(void* arkode_mem, int degree);

// Error and diagnostic files
int OSStepSetDiagnostics(void* arkode_mem, FILE* diagfp);

int OSStepSetErrFile(void* arkode_mem, FILE* errfp);

int OSStepSetErrHandlerFn(void* arkode_mem, OSErrHandlerFn ehfun, void* eh_data);

// initial size, and stopping rules
int OSStepSetInitStep(void* arkode_mem, realtype hin)

int OSStepSetMaxHnilWarns(void* arkode_mem, int mxhnil)

int OSStepSetMaxNumSteps(void* arkode_mem, long int mxsteps)

int OSStepSetMaxStep(void* arkode_mem, realtype hmax)

int OSStepSetMinStep(void* arkode_mem, realtype hmin)

int OSStepSetMinStep(void* arkode_mem, realtype hmin)

int OSStepSetInterpolateStopTime(void* arkode_mem, booleantype interp)

int OSStepClearStopTime(void* arkode_mem)

// User data passed into functions
int OSStepSetUserData(void* arkode_mem, void* user_data)

// WKB solver
int OSStepWKBSetOrder(void* arkode_mem, int ord)

// Use only RK or WKB (Mostly for debugging purposes probably)
// OSCODE_MODE is an enum of {RK, WKB, BOTH} with default of BOTH
int OSStepSetExplicit(void* arkode_mem, OSCODE_MODE mode)

// Set order and tables for WKB solver 
int OSStepWKBSetTables(void* arkode_mem, int o, OSodeButcherTable Bi)



// Set order and tables for RK solver 
int OSStepRKSetTables(void* arkode_mem, int o, OSodeButcherTable Bi)

int OSStepRKSetTableNum(void* arkode_mem, OSODE_ERKTableID etable)
```
...
========================================================   ======================================  ========
Set a custom time step adaptivity function                 :c:func:`OSStepSetAdaptivityFn()`      internal
Choose an existing time step adaptivity method             :c:func:`OSStepSetAdaptivityMethod()`  0
Explicit stability safety factor                           :c:func:`OSStepSetCFLFraction()`       0.5
Time step error bias factor                                :c:func:`OSStepSetErrorBias()`         1.5
Bounds determining no change in step size                  :c:func:`OSStepSetFixedStepBounds()`   1.0  1.5
Maximum step growth factor on convergence fail             :c:func:`OSStepSetMaxCFailGrowth()`    0.25
Maximum step growth factor on error test fail              :c:func:`OSStepSetMaxEFailGrowth()`    0.3
Maximum first step growth factor                           :c:func:`OSStepSetMaxFirstGrowth()`    10000.0
Maximum allowed general step growth factor                 :c:func:`OSStepSetMaxGrowth()`         20.0
Minimum allowed step reduction factor on error test fail   :c:func:`OSStepSetMinReduction()`      0.1
Time step safety factor                                    :c:func:`OSStepSetSafetyFactor()`      0.96
Error fails before MaxEFailGrowth takes effect             :c:func:`OSStepSetSmallNumEFails()`    2
Explicit stability function                                :c:func:`OSStepSetStabilityFn()`       none
========================================================   ======================================  ========
